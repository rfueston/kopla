...
export default function PickLane() {
    ...
    const getParentData = async (data) => {
        if (data && data.text !== "") {
            ...
            addToQueue(documents[0].parentName, queueStudentIdsArray, data.text, "QRScanner");
        }
    };

    const addToQueue = async (queueParentId, queueStudentIdsArray, parentEmail, type = "other") => {
        try {

            const collectionRef = collection(db, "queue");
            const q = query(collectionRef, where("parent_email_id", "==", parentEmail));
            const snapshot = await getDocs(q);
            const documents = snapshot.docs.map((doc) => doc.data());
    ...
    const startGPSServices = () => {
        let intervalAmount;

        alert("Starting GPS Services");
        ...
        const updateLocation = async () => {
            try {
                ...
                try {
                    ...
                    if (docSnap.exists()) {
                        const {lat: geoLat, lng: geoLng} = docSnap.data();
                        const tolerance = 0.0500; // added geofencing area

                        if (Math.abs(userLat - geoLat) < tolerance && Math.abs(userLng - geoLng) < tolerance) {
                            alert("Location matches GeoMap location");
                            await setParentToQueue(profileData.email);
                            clearInterval(intervalAmount);
                            intervalAmount = undefined;  // reset to prevent looping
        ...
        if (typeof intervalAmount === 'undefined') {
            intervalAmount = setInterval(updateLocation, 5000);
        }
    };
    ...
    const setParentToQueue = async (data) => {
        try {
            const collectionRef = collection(db, "Pairs");
            const q = query(collectionRef, where("parentEmail", "==", data));
            const snapshot = await getDocs(q);
            ...
            if (!snapshot.empty) {
                ...
                await addToQueue(documents[0].parentName, queueStudentIdsArray, data);

                alert("Successfully added to the queue!");
            } else {
                alert("No documents found for the provided parentEmail.");
            }
        } catch (error) {
            console.error('Error setting parent to queue:', error);
            alert("An error occurred while adding to the queue. Please try again.");
        }
    };
                ...
                {!profileData.isParent && (
                    <div>
                        ...
                        <center>
                            <button
                                className={styles.outerButtons}
                                onClick={() => setParentToQueue(profileData.email)}
                            >
                                I'm Here!
                            </button>
                        </center>
