...
import styles from "../src/app/pickup_lane/pickup.module.css"; // Import the CSS
    ...
    <div key={isWebcamActiveRef.current}>
      {isWebcamActiveRef.current && (
        <>
          ...
          <button className={styles.buttons} onClick={() => { stopWebcam(); reloadPage(); }}>Stop Scanning</button>
        </>
      )}
      {!isWebcamActiveRef.current && (
        <button className={styles.buttons} onClick={startWebcam}>Scan QR Codes</button>
      )}
    </div>
  );
};
